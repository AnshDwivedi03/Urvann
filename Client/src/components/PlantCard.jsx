  ...
  window.scrollTo({ top: 0, behavior: "smooth" });
    ...
    >
      ...
      <div className="p-4 sm:p-5">
        <h3 className="text-lg font-medium mb-2">{plant.name}</h3>
        <p className="text-sm text-muted-foreground mb-2">{plant.description}</p>
        <p className="text-sm text-muted-foreground mb-2">{plant.categories}</p>
        <div className="flex items-center text-sm text-muted-foreground">
          <img src={assets.location_icon} alt="Location" className="h-4 mr-2" />
          <span>{plant.location || "Delhi"}</span>
        </div>
      </div>
    </div>
  );
